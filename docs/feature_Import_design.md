...
`feature/CheckerImport`에서 이미 구현한 CheckerUnit 쪽(2~3.2절 내용)은 **코드 레벨 충돌은 없다** — PR #76은 Tokenizer만 건드렸다. **(2026-07-10 정정)** 이후 PR #86(`Feature/assembler unit/import`)에서 `ImportStatementParser`가 실제로 추가되어, `import "경로" alias 별칭;` 문이 이제 진짜로 `ImportStmtNode`(children = `[path: StringLiteralNode, alias: IdentifierNode]`)로 파싱되고 `CheckerUnit::Visit(const ImportStmtNode&)`가 실행 경로에서 동작한다 — 더 이상 "죽은 코드"가 아니다. 다만 `ExecutorUnit`은 여전히 `Visit(const ImportStmtNode&)`를 override하지 않아 기본 `Traverse()`로만 지나가고, 실제 모듈 로딩/네임스페이스 바인딩(3.3절의 `ModuleLoader`)은 아직 없다 — 즉 "import 문 자체는 파싱·검사된다"와 "alias가 실제로 뭔가를 가리킨다"는 여전히 별개다. 아래 항목들은 이 정정을 반영해 다시 읽을 것.
...
## 6. (가정) 완료 상태 스냅샷 — "다 구현됐다면" 시나리오

> **주의**: 이 절은 실제 구현이 아니라, 위 오픈 이슈(특히 5번)가 전부 결정되고 `ModuleLoader`/실행부까지 이어 붙었다고 **가정했을 때**의 최종 그림을 미리 그려본 것이다. 실제로 코드가 이렇게 동작하는지는 항상 `ExecutorUnit.cpp`에서 `Visit(const ImportStmtNode&)` override 존재 여부로 재확인할 것 — 이 절 작성 시점(2026-07-10)에는 아직 없다.

이 시나리오는 오픈 이슈 5번을 **(a) alias 네임스페이싱**으로 결정한 것을 전제로 한다(슬라이드 요구사항인 `math.add(1, 2)` 형태의 멤버 접근을 지원하기 위함).

### 6.1 파이프라인 최종 형태

- **Tokenizer**: 0절의 스플라이스 방식을 계속 쓰되, 스플라이스 경계에 **마커 토큰**(예: `ImportBegin`/`ImportEnd`, 파일 경로/알파벳 alias 이름을 payload로 들고 있는 합성 토큰)을 추가로 삽입한다. 순환 import/파일없음 감지는 그대로 Tokenizer가 담당(변경 없음).
- **AssemblerUnit / ImportStatementParser**: `import ... alias X;` 5개 토큰에 더해, 그 뒤로 이어지는 `ImportBegin(X) ... ImportEnd(X)` 구간을 통째로 `ImportStmtNode`의 자식으로 흡수한다 — 즉 스플라이스된 선언들이 이제 전역 최상위 토큰이 아니라 `ImportStmtNode.children`(예: `[path, alias, declarations...]`) 아래로 들어간다.
- **CheckerUnit**: 기존 3.2절 체크(같은/상위 스코프 중복 import, alias 충돌, 반복문 내부 금지)는 그대로 유지. 추가로 `ImportStmtNode`의 하위 선언들을 **alias 이름으로 스코프된 별도 네임스페이스**로 취급해, `math.add`처럼 `MemberAccessExprNode`를 통한 접근만 허용하고 `add`라는 이름이 바깥 스코프에 그냥 노출되지는 않는지 검사.
- **ExecutorUnit**: `Visit(const ImportStmtNode&)`를 새로 override — 하위 선언들을 실행해 함수/전역 변수를 수집한 뒤, `ModuleObject`(alias 이름 → export 맵) 하나로 감싸 alias 식별자를 **현재 스코프에만** 바인딩한다. `MemberAccessExprNode`가 `ModuleObject`를 대상으로 오면 export 맵에서 이름을 찾아 호출/참조한다(클래스 인스턴스의 `.` 접근과 같은 코드 경로를 공유하되, 대상이 `ModuleObject`인 경우로 분기).
- 파일 존재/순환 import는 여전히 Tokenizer가 가장 먼저 잡으므로 3.3절에서 말한 실행 시점 `ModuleLoader`의 "파일 존재/순환 감지" 역할은 사실상 필요 없어진다 — `ModuleLoader`가 남긴다면 역할은 "이미 토큰화·파싱된 하위 트리를 실행해 export 맵을 만드는 것"으로 축소된다.

### 6.2 오픈 이슈 5번 이후 남는 결정들 (이 가정 시나리오에서 새로 열리는 질문)

1. 같은 alias로 서로 다른 스코프에서 두 번 import하면 export 맵을 매번 새로 실행할지, 최초 1회 실행 후 캐시(파일 경로 기준)할지 — import "실행"이라는 개념이 다시 생기므로 5절 이슈 2번이 재부상한다.
2. `ModuleObject`를 값으로 다른 변수에 대입하거나 함수 인자로 넘기는 것을 허용할지(1급 값 취급 여부).
3. export 대상을 명시적으로 선언(`export func add(...)`처럼)할지, 파일 내 모든 최상위 `func`/`var`를 암묵적으로 export할지 — 이 절의 가정은 후자(암묵적 전체 export)를 전제로 함.

### 6.3 이 절이 실제로 유효해지는 조건

- `ExecutorUnit.cpp`에 `void Visit(const ImportStmtNode& node) override;`가 추가되고,
- `Tokenizer::ResolveImports`가 스플라이스 경계 마커를 남기도록 바뀌며,
- `ImportStatementParser`가 그 마커 구간을 `ImportStmtNode`의 자식으로 흡수하도록 바뀌어야

비로소 이 절의 그림이 현실이 된다. 그 전까지는 0~5절이 현재 진실이다.
