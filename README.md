...
- [실행 제약 사항 및 특이 동작](#실행-제약-사항-및-특이-동작)
...
| 출력 | `print expr;` |
| 키워드 대소문자 | 예약어(`if`, `var`, `true` 등)는 대소문자를 구분하지 않아 `IF`, `Var`도 동일하게 동작하지만, 식별자는 대소문자를 구분합니다(`myVar` ≠ `myvar`). |
| 주석 | 지원하지 않습니다. `//`, `/* */` 모두 파싱 에러가 됩니다. |
| 문자열 리터럴 | 이스케이프 시퀀스를 지원하지 않습니다(`\"`, `\n` 등 불가). 문자열 안에 큰따옴표 자체를 포함할 수 없습니다. |
| 변수 타입 | 동적 타입입니다. 재대입 시 타입이 자유롭게 바뀔 수 있습니다: `var a = 1; a = "hello";` |
| `for` | C 스타일 3구문(`init; condition; update`) 형태만 지원합니다. `for-in`/`for-of` 같은 순회 문법은 없습니다. |
| `instanceof` | 우변은 식으로 평가되지 않고 토큰 그대로 클래스 이름으로 취급됩니다(`x instanceof SomeExpr()` 같은 형태는 불가). |

## 실행 제약 사항 및 특이 동작

일반적인 스크립팅 언어와 다르게 동작하는, 실제로 코드를 작성할 때 부딪히기 쉬운 부분들입니다.

| 항목 | 설명 |
|---|---|
| REPL 블록 입력 | `{`로 시작하면 대응하는 `}`가 나올 때까지 입력을 계속 받은 뒤, 블록이 닫히면 한 번에 파싱/실행합니다. |
| REPL `if`(`else` 없음) | `else`가 없는 `if` 블록은 실행되기 전에 빈 줄(엔터)을 한 번 더 입력해야 합니다. REPL이 다음 줄에 `else`가 이어질지 미리 알 수 없기 때문입니다. |
| import 대상 파일의 실행문 | import되는 파일에서는 선언(함수/전역 변수/import)만 반영되고, 그 외 최상위 실행문(예: `print`)은 무시됩니다. |
| import 중복 | 같은 경로를 다른 alias로 다시 import해도 컴파일 에러입니다(자동으로 dedupe되지 않습니다). |
| `instanceof` 좌변 | 좌변이 인스턴스가 아니면 에러를 던지지 않고 그냥 `false`로 평가됩니다. |
