...
protected:
	...
	std::string RunAndCapture(SyntaxNode& program) {
		testing::internal::CaptureStdout();
		executor.Execute(program);
		return testing::internal::GetCapturedStdout();
	}

	void ExpectRuntimeError(SyntaxNode& program, const std::string& expectedSubstr) {
		try {
			executor.Execute(program);
...
TEST_F(ExecutorUnitTest, Execute_UndefinedVariableAssignment_ThrowsWithLineMessage) {
	...
	ExpectRuntimeError(program, "[1번째줄] 미정의된 변수 'x'");
}
