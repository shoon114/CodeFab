...
using Value_t = std::variant<double, std::string, bool, std::shared_ptr<FunctionObject>, std::monostate,
    std::shared_ptr<ArrayObject>, std::shared_ptr<InstanceObject>, std::shared_ptr<ModuleObject>>;

struct FunctionObject {
    std::vector<std::string> parameters;
    const SyntaxNode* body = nullptr;
    // import된 함수는 파일의 전역 선언을 별도 모듈 스코프로 캡처한다.
    std::shared_ptr<std::unordered_map<std::string, Value_t>> closure;
};
...
// import "file" alias name; 실행 결과로 현재 스코프에 들어가는 네임스페이스 값.
struct ModuleObject {
    std::string path;
    std::shared_ptr<std::unordered_map<std::string, Value_t>> members;
};
