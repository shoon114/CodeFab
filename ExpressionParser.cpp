...
std::unique_ptr<SyntaxNode> ExpressionParser::ParseCallExpr(const TokenList& tokenList, size_t& pos, std::unique_ptr<SyntaxNode> callee) {
	if (callee->type != NodeType::Identifier && callee->type != NodeType::MemberAccessExpr) {
		throw std::runtime_error("Only an identifier or member can be called at line " + std::to_string(callee->token.line));
	}
...
std::unique_ptr<SyntaxNode> ExpressionParser::ParseMemberAccessExpr(const TokenList& tokenList, size_t& pos, std::unique_ptr<SyntaxNode> target) {
	pos++; // '.'
	if (PeekType(tokenList, pos) != TokenType::Identifier) {
		throw std::runtime_error("Expected member name after '.' at line " + std::to_string(target->token.line));
	}
